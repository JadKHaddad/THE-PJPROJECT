...
fn main() {
    unsafe {
        ...
        (*acc_cfg.as_mut_ptr()).id = pj_str(id.as_ptr() as *mut i8);

        let uri = CString::new(&*format!("sip:{}", SIP_DOMAIN)).expect(CSTRING_NEW_FAILED);
        (*acc_cfg.as_mut_ptr()).reg_uri = pj_str(uri.as_ptr() as *mut i8);

        (*acc_cfg.as_mut_ptr()).cred_count = 1;

        let sip_domain = CString::new(SIP_DOMAIN).expect(CSTRING_NEW_FAILED);
        (*acc_cfg.as_mut_ptr()).cred_info[0].realm = pj_str(sip_domain.as_ptr() as *mut i8);

        let digest = CString::new("digest").expect(CSTRING_NEW_FAILED);
        (*acc_cfg.as_mut_ptr()).cred_info[0].scheme = pj_str(digest.as_ptr() as *mut i8);

        let username = CString::new(SIP_USER).expect(CSTRING_NEW_FAILED);
        (*acc_cfg.as_mut_ptr()).cred_info[0].username = pj_str(username.as_ptr() as *mut i8);

        (*acc_cfg.as_mut_ptr()).cred_info[0].data_type =
            pjsip_cred_data_type_PJSIP_CRED_DATA_PLAIN_PASSWD as i32;

        let password = CString::new(SIP_PASSWD).expect(CSTRING_NEW_FAILED);
        (*acc_cfg.as_mut_ptr()).cred_info[0].data = pj_str(password.as_ptr() as *mut i8);
